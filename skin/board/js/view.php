<link href="<?php echo($site_home); ?>/assets/css/themes/blackboard.css" rel="stylesheet" type="text/css" media="screen">
	...
	<?php
		...
		if (is_array($content)) {
			...
			for($i = 0; $i < COUNT($content); $i++) {
				if(strpos($content[$i], 'text:') !== false) {
					$content[$i] = str_replace('text:', '', $content[$i]);
					$contents .= nl2br(keywordHightlight($all_search, "highlight", $content[$i]));
				}

				if(strpos($content[$i], 'code:') !== false) {
					//$content[$i] = str_replace(array("code:"),'"code:".',$content[$i]);
					
					$content[$i] = str_replace('code:', '', $content[$i]);
					//$content[$i] = '<pre><code data-language="javascript">'.$content[$i].'</code></pre>';
					$contents .= '<pre><code data-language="javascript">';
					$contents .= keywordHightlight($all_search, "highlight", $content[$i]);
					$contents .= '</code></pre>';
					$contents = str_replace(array('<pre>'."\r\n",'<pre>'."\r",'<pre>'."\n"), '<pre>', $contents);
					$contents = str_replace(array('<code data-language="javascript">'."\r\n",'<code data-language="javascript">'."\r",'<code data-language="javascript">'."\n"), '<code data-language="javascript">', $contents);
				}

				if(strpos($content[$i], 'linkUrl:') !== false) {
					$content[$i] = str_replace('linkUrl:', '', $content[$i]);
					$contents .= '<br /><a href="'.$content[$i].'" target="blank">'.keywordHightlight($all_search, "highlight", $content[$i]).'</a>';
				}
...
<!-- /.modal -->
<script src="<?php echo($site_home); ?>/assets/js/rainbow.js"></script>
<script src="<?php echo($site_home); ?>/assets/js/language/generic.js"></script>
<script src="<?php echo($site_home); ?>/assets/js/language/javascript.js"></script>
