...
?>
	<li>
		<p class="<?php echo('text-'.$rows['img_align']); ?>">
			<a href="#" data-toggle="modal" data-target="#imgModal" data-id="<?php echo($rows['id']); ?>">
...
<!-- Modal -->
<?php
	$sql = "SELECT id, dir, file_name, origin_name FROM bbs_files 
			WHERE (file_type LIKE '%png%' OR file_type LIKE '%jpeg%' OR file_type LIKE '%gif%') AND document_id = $id";
	$result = $db -> query($sql);
	$count_img = $db -> num_rows();
?>
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Modal Header<?php echo($count_img); ?></h4>
			</div>
			<div class="modal-body">
			<?php
				if($count_img == 1) {
					$img_files = $db -> fetch_assoc();
			?>
				<img src="<?php echo($site_home.'/'.$img_files['dir'].$img_files['file_name']); ?>" alt="<?php echo($img_files['origin_name']); ?>" class="img-responsive">
			<?php
				} else {
					while($img_files = $db -> fetch_assoc()) {
			?>
				<img src="<?php echo($site_home.'/'.$img_files['dir'].$img_files['file_name']); ?>" alt="<?php echo($img_files['origin_name']); ?>" class="img-responsive">
			<?php
					}
