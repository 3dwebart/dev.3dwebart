...
?>
<div class="detail-contents edit-forms col-xs-12">
    <!-- 글 번호 상태 유지 -->
...
<? } ?>
    ...
    <div>
        ...
        <div class="form-group ex-group">
            ...
            <div class="col-sm-3">
                <input type="text" class="form-control" id="ex1" name="ex1[]"
                    value="<?php echo($ex1[$i]); ?>" placeholder="상세 내역의 타이틀을 입력하세요.">
            </div>
            <label for="ex1" class="col-sm-2 control-label">설명(내용)</label>
            <div class="col-sm-3">
                <input type="text" class="form-control" id="ex2" name="ex2[]"
                    value="<?php echo($ex2[$i]); ?>" placeholder="상세 내역의 내용을 입력하세요.">
            </div>
            <div class="col-sm-2">
                <button type="button" class="btn btn-info ex-add">
        ...
        <div class="form-group ex-group">
            ...
            <div class="col-sm-3">
                <input type="text" class="form-control" id="ex1" name="ex1[]"
                    value="<?php echo($ex1); ?>" placeholder="상세 내역의 타이틀을 입력하세요.">
            </div>
            <label for="ex1" class="col-sm-2 control-label">설명(내용)</label>
            <div class="col-sm-3">
                <input type="text" class="form-control" id="ex2" name="ex2[]"
                    value="<?php echo($ex2); ?>" placeholder="상세 내역의 내용을 입력하세요.">
            </div>
            <div class="col-sm-2">
                <button type="button" class="btn btn-info ex-add">
                    <i class="fa fa-plus"></i>
    ...
    jQuery(document).on('click', '.ex-add', function() {
        current_ex = '<div class="form-group ex-group">' + 
            ...
            '<div class="col-sm-3">' + 
                '<input type="text" class="form-control" id="ex1" name="ex1[]">' + 
            '</div>' + 
            '<label for="ex2" class="col-sm-2 control-label">' + 
            '<p class="form-control-static">' + 
            '설명' + 
            '</p>' + 
            '</label>' + 
        ...
        if(jQuery('.ex-group').length > 1) {}
    });
    console.log('ex-group 갯수 = ' + jQuery('.ex-group').length);
    if(jQuery('.ex-group').length > 1) {
        jQuery('.ex-group').eq(0).siblings().find('.ex-add').after('<button type="button" class="btn btn-danger ex-del">' + 
                    '<i class="fa fa-minus"></i>' + 
                '</button>');
    }
