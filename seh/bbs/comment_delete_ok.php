...
<?
    ...
    if (!$session_id) {
        // 삭제 SQL
        $sql = "DELETE FROM bbs_comments WHERE id=%d AND pwd=password('%s')";
        $result = $db -> query($sql, array($comment_id, $pwd));
    } else {
        // 삭제 SQL
        $sql = "DELETE FROM bbs_comments WHERE id=%d AND member_id=%d";
        $result = $db -> query($sql, array($comment_id, $session_u_id));
    }
