...
<?
    ...
    if (!$session_u_id) {
        $sql = "UPDATE bbs_comments SET
                    writer='%s', email='%s', content='%s', edit_date=now()
                WHERE id=%d AND pwd=password('%s')";

        $result = $db -> query($sql, array($writer, $email, $content,
                                            $comment_id, $pwd));
    } else {
        $sql = "UPDATE bbs_comments SET
                    writer='%s', email='%s', content='%s', edit_date=now()
        ...
        $result = $db -> query($sql, array($session_user_name, $session_email, $content,
                                            $comment_id, $session_u_id));
    }
