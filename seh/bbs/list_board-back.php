...
<table class="table">
	...
	?>
		<tr>
			...
			<td>
				<a href='read.php?bbs_id=<?=$bbs_id?>&id=<?=$row['id']?>'><?=htmlspecialchars($row['subject'])?></a>
	<?php
	...
	?>
			...
			<td class='text-center'><?=htmlspecialchars($row['writer'])?></td>
...
<nav class='text-center'>
	...
	<?php if ($prev_page > 0) { ?>
		<li>
			<a href="index.php?bbs_id=<?=$bbs_id?>&page=<?=$prev_page?>&search=<?=$search?>">
	...
	?>
		<!-- 다음 그룹 -->
	<?php if ($next_page > 0) { ?>
		<li><a href="index.php?bbs_id=<?=$bbs_id?>&page=<?=$next_page?>&search=<?=$search?>">&raquo;</a></li>
	<?php } else { ?>
		<li class='disabled'>
			<a href="#" onclick="return false">
