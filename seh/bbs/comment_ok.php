...
<?
    ...
    if (!$session_u_id) {
        $sql = "INSERT INTO bbs_comments (
                    document_id, writer, pwd, email, content, reg_date, edit_date
                ) VALUES (
                    %d, '%s', password('%s'), '%s', '%s', now(), now()
                )";

        $result = $db -> query($sql, array($id, $writer, $pwd, $email, $content));
    } else {
        $sql = "INSERT INTO bbs_comments (
        ...
        $result = $db -> query($sql, array($id, $session_u_id, $session_user_name, $session_u_id, 
                                    $session_email, $content));
    }
