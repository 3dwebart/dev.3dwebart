<?php
	...
	SET bo_group_id = %d, bo_group_name = '%s', title = '%s', type = '%s', l_level = '%s', r_level = '%s', w_level = '%s', edit_date = now() 
	WHERE id = %d";

	$result = $db -> query($sql, array($bo_group_id, $bo_group_name, $title, $type, $l_level, $r_level, $w_level, $board_num));
	...
	if (is_array($file_info_t)) {
		...
		if ($db -> fetch_row() > 0) {
			...
			if (is_array($file_info_t)) {
				$db -> query($sql, array(
					$board_num, $file_info_t['dir'], $file_info_t['file_name'], 
					$file_info_t['file_type'], $file_info_t['file_size'], 
					$file_info_t['origin_name'], $file_info_t['full_path']
				));
			}
	...
	if (is_array($file_info_b)) {
		...
		if ($db -> fetch_row() > 0) {
			...
			if (is_array($file_info_b)) {
				$db -> query($sql, array(
					$board_num, $file_info_b['dir'], $file_info_b['file_name'], 
					$file_info_b['file_type'], $file_info_b['file_size'], 
					$file_info_b['origin_name'], $file_info_b['full_path']
				));
			}
	...
	*/
	...
	redirect($admin_home.'/adm_board_list.php', '게시판 정보가 수정되었습니다.', FALSE);
