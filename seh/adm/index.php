...
<script>
	$(function() {
		$('.pw-form').keypress(function(event){
			if(event.which == 13){
		...
		$('.id-form').keypress(function(event){
			if(event.which == 13){
				event.preventDefault();
				$('.MyForm').attr('action', 'adm_check_ok.php');
				$('.MyForm').attr('method', 'post');
...
<!--
		<div class="container">
			...
			<form class="form-horizontal" role="form" method="post" action="adm_check_ok.php" enctype="multipart/form-data">
				<div class="form-group">
					<div class="col-md-3">
						<label for="adm_id" class="label-control">아이디</label>
					</div>
					<div class="col-md-9">
						<input type="text" name="adm_id" id="adm_id" class="form-control" />
					</div>
				</div>
				<div class="form-group">
					<div class="col-md-3">
					...
					<div class="col-md-9">
						<input type="password" name="adm_pw" id="adm_pw" class="form-control" />
					</div>
				</div>
			...
			<div>
				<div class="list-group">
					<div class="col-md-12">
						<h3 class="alert alert-info">최근 가입자</h3>
						<table class="member-list table table-striped table-bordered">
							...
							<tbody>
					<?
						$sql = "SELECT id, user_id, user_name, user_nic, email, tel, reg_date FROM members ORDER BY id DESC LIMIT 0, 2";
						$result = $db -> query($sql);
					...
					<div class="col-md-12">
						<h3 class="alert alert-info">최근 게시물</h3>
					</div>
				</div>
			</div>
			<? } ?>
