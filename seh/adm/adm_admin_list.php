					...
					<tr>
						...
						<td class="text-center">
							<select name="adm_level[]" id="adm_level" class="form-control">
								<option value=1 <? if ($row['adm_level'] == 1) { ?>selected<?}?>>1</option>
								<option value=2 <? if ($row['adm_level'] == 2) { ?>selected<?}?>>2</option>
								<option value=3 <? if ($row['adm_level'] == 3) { ?>selected<?}?>>3</option>
								<option value=4 <? if ($row['adm_level'] == 4) { ?>selected<?}?>>4</option>
								<option value=5 <? if ($row['adm_level'] == 5) { ?>selected<?}?>>5</option>
							</select>
						</td>
						<td><?=$row['adm_id']?></td>
						<td>
							<input type="text" class="form-control" name="adm_name[]" <? if ($row['adm_name']) {?>value="<?=$row['adm_name']?>"<?}?> />
						</td>
						<td>
							<input type="text" class="form-control" name="adm_nic[]" <? if ($row['adm_nic']) {?>value="<?=$row['adm_nic']?>"<?}?> />
						</td>
						<td>
						...
						<td class="text-center">
							<div class="form-group form-inline" role="form">
								<a href="adm_admin_edit.php?admin_id=<?=$admin_id?>" class="btn btn-success">
	...
	<div class="page-content">
		...
		<table class="member-list table table-striped table-bordered table-responsive">
			<thead>
				<tr>
					<th><input name="all_chk" type="checkbox" class="checkbox all_chk"></th>
					<th class="text-center"><a href="adm_admin_list.php?sort=adm_level&orderby=<?=$orderby?>">권한</a></th>
					<th class="text-center"><a href="adm_admin_list.php?sort=adm_id&orderby=<?=$orderby?>">아이디</a></th>
					<th class="text-center"><a href="adm_admin_list.php?sort=adm_name&orderby=<?=$orderby?>">이름</a></th>
					<th class="text-center"><a href="adm_admin_list.php?sort=adm_nic&orderby=<?=$orderby?>">닉네임</a></th>
					<th class="text-center"><a href="adm_admin_list.php?sort=email&orderby=<?=$orderby?>">이메일</a></th>
					<th class="text-center"><a href="adm_admin_list.php?sort=tel&orderby=<?=$orderby?>">전화번호</a></th>
					<th class="text-center"><a href="adm_admin_list.php?sort=reg_date&orderby=<?=$orderby?>">가입일</a></th>
					<th class="text-center">
						<a href="adm_admin_add.php" class="btn btn-primary">
							<i class="glyphicon glyphicon-plus"></i>
						</a>
		...
		<nav class='text-center'>
			...
			<?php if ($prev_page > 0) { ?>
				<li>
					<a href="adm_admin_list.php?admin_id=<?=$admin_id?>&page=<?=$prev_page?><?if ($separation) {?>&separation=<?=$separation?><?}?>&search=<?=$search?>">
			...
			?>
				<li><a href="adm_admin_list.php?admin_id=<?=$admin_id?>&page=<?=$i?><?if ($separation) {?>&separation=<?=$separation?><?}?>&search=<?=$search?>"><?=$i?></a></li>
			<?php
					}
				}
			?>
				<!-- 다음 그룹 -->
			<?php if ($next_page > 0) { ?>
				<li><a href="adm_admin_list.php?admin_id=<?=$admin_id?>&page=<?=$next_page?><?if ($separation) {?>&separation=<?=$separation?><?}?>&search=<?=$search?>">&raquo;</a></li>
			<?php } else { ?>
				<li class='disabled'>
					<a href="#" onclick="return false">
