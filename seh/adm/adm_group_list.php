...
<table class="group-list table table-striped table-bordered table-responsive">
	...
	<thead>
		<tr>
			...
			<th class="text-center" rowspan="2" valign="top"><a href="adm_group_list.php?sort=bo_group_id	&orderby=<?=$orderby?>">게시판 그룹 아이디</a></th>
			<th class="text-center" rowspan="2" valign="top"><a href="adm_group_list.php?sort=bo_group_name&orderby=<?=$orderby?>">게시판 그룹 이름</a></th>
			<th class="text-center" rowspan="2"><a href="adm_group_list.php?sort=reg_date&orderby=<?=$orderby?>">게시판 생성일</a></th>
...
?>
			<tr>
				...
				<td>
					<input type="hidden" class="form-control" name="bo_group_id[]" <? if ($row['bo_group_id']) {?>value="<?=$row['bo_group_id']?>"<?}?> />
					<?=($row['bo_group_id'])?>
				</td>
				<td>
					<input type="text" class="form-control" name="bo_group_name[]" <? if ($row['bo_group_name']) {?>value="<?=$row['bo_group_name']?>"<?}?> />
				</td>
				<td><?=$row['reg_date']?></td>
				<td class="text-center buttons">
					<div class="form-group form-inline" role="form">
						<input type="hidden" name="id" value="<?=$row['id']?>">
						<a href="<?=$admin_home?>/adm_board_list.php?group_id=<?=$group_id?>" class="btn btn-warning">
							<i class="glyphicon glyphicon-eye-open"></i>
						</a>
						<a href="adm_group_edit.php?group_id=<?=$group_id?>" class="btn btn-success">
...
<nav class='text-center'>
	...
	<?php if ($prev_page > 0) { ?>
		<li>
			<a href="adm_group_list.php?group_id=<?=$group_id?>&page=<?=$prev_page?><?if ($separation) {?>&separation=<?=$separation?><?}?>&search=<?=$search?>">
	...
	?>
		<li><a href="adm_group_list.php?group_id=<?=$group_id?>&page=<?=$i?><?if ($separation) {?>&separation=<?=$separation?><?}?>&search=<?=$search?>"><?=$i?></a></li>
	<?php
			}
		}
	?>
		<!-- 다음 그룹 -->
	<?php if ($next_page > 0) { ?>
		<li><a href="adm_group_list.php?group_id=<?=$group_id?>&page=<?=$next_page?><?if ($separation) {?>&separation=<?=$separation?><?}?>&search=<?=$search?>">&raquo;</a></li>
	<?php } else { ?>
		<li class='disabled'>
			<a href="#" onclick="return false">
